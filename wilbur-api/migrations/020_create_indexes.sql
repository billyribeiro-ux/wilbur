...
CREATE INDEX IF NOT EXISTS idx_chatmessages_room_created_desc ON chatmessages (room_id, created_at DESC);

-- Alerts: paginate alerts in a room (newest first)
CREATE INDEX IF NOT EXISTS idx_alerts_room_created_desc ON alerts (room_id, created_at DESC);

-- Media tracks: find active tracks in a room
CREATE INDEX IF NOT EXISTS idx_media_tracks_room_active ON media_tracks (room_id, is_active);

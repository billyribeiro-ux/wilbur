<script lang="ts">
	...
	import { XIcon, CheckCircleIcon, XCircleIcon, WarningIcon, InfoIcon } from 'phosphor-svelte';
	...
	function getIcon(type: Toast['type']) {
		switch (type) {
			...
			case 'warning':
				return { icon: WarningIcon, class: 'text-yellow-400' };
			case 'info':
			default:
				return { icon: InfoIcon, class: 'text-blue-400' };
		}
	}

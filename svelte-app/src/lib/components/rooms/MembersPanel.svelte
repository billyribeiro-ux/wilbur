<script lang="ts">
	...
	function getRoleIcon(role: string): { icon: Component; class: string } {
		switch (role) {
			...
			case 'host':
				return { icon: CrownIcon, class: 'text-purple-400' };
			case 'moderator':
				return { icon: ShieldIcon, class: 'text-blue-400' };
			default:
				return { icon: UserIcon, class: 'text-surface-400' };
		}
	}
...
<div class="flex h-full flex-col">
	...
	<div class="flex items-center justify-between border-b border-surface-700 px-4 py-3">
		<div class="flex items-center gap-2">
			<UsersIcon class="h-5 w-5 text-primary-400" />
		...
		{#if sortedMembers.length === 0}
			<div class="flex h-full flex-col items-center justify-center text-center">
				<div class="rounded-full bg-surface-800 p-4">
					<UsersIcon class="h-8 w-8 text-surface-500" />
				</div>

<script lang="ts">
	...
	import { PlusIcon, MagnifyingGlassIcon, UsersIcon, ChatCircleIcon, BellIcon, GearIcon, SignOutIcon, TrendUpIcon, UserIcon } from 'phosphor-svelte';
		...
		<div class="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
			...
			<div class="flex items-center gap-4">
				...
				<div class="relative hidden sm:block">
					<MagnifyingGlassIcon class="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-surface-500" />
					<input
						type="text"
				...
				<div class="relative group">
					<button class="flex items-center gap-2 rounded-lg border border-surface-600 px-3 py-2 hover:bg-surface-800 transition">
						<div class="h-8 w-8 rounded-full bg-primary-500/20 flex items-center justify-center">
							<UserIcon class="h-4 w-4 text-primary-400" />
						</div>
		...
		{:else if filteredRooms.length === 0}
			...
			<div class="flex flex-col items-center justify-center py-16 text-center">
				<div class="flex h-20 w-20 items-center justify-center rounded-full bg-surface-800">
					<ChatCircleIcon class="h-10 w-10 text-surface-500" />
				</div>
					...
					>
						<div class="flex items-start justify-between">
							...
							>
								...
								{:else}
									<TrendUpIcon class="h-6 w-6 text-primary-400" weight="duotone" />
								{/if}
							</div>
